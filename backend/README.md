...
Completed games receive a random 128-bit replay identifier. `GET /api/replays/{replayID}` returns rankings, ties, revealed Model Says boards/matches or trivia solutions/correctness/selections, and per-round score deltas. It deliberately omits tokens, normalized guesses, accepted aliases, provider/model/prompt metadata, audits, judge records, and raw provider content. `POST /api/rooms/{code}/play-again` is host-token protected and creates a distinct lobby with copied ruleset/pacing settings and a new host identity; no questions, gameplay rows, scores, or credentials are reused.
...
Room creation accepts individual simultaneous, team, or sequential mode, 1–`MAX_ROUNDS` rounds, a 15–120 second answer timer, an `AVAILABLE_LOCALES` locale, and the prediction model configured by `DEFAULT_PREDICTION_MODEL` (default `gpt-5.6-luna`). Omitted settings retain `DEFAULT_ROUNDS` (10 by default) and 45 seconds. Room names must contain 3–48 Unicode characters and display names 2–24; control characters are rejected. Route room codes must be six characters from the invite-code alphabet.
...
Generation gets two attempts. If either attempt fails or returns invalid content, the server falls back to an unused curated bank entry for that round. Separate reviewed English banks contain ten unique Model Says, Open Trivia, and Choice Trivia rounds, matching the default round count. Trivia provider output uses the strict `trivia-v1` schema and atomically includes the question, canonical answer, aliases or four options/correct ID, base score, explanation, and source note. The same provider-independent content validator and integrity hash run before persistence. Provider failures do not block an offline game; if the model and curated paths both fail, start/advance returns HTTP 503 with a retryable content-unavailable message. Audits record actual provider metadata, purpose, path, outcome, usage, and cost; optional bounded raw-response capture remains disabled by default.
