...
func (server *Server) writeDomainError(writer http.ResponseWriter, err error) {
	...
	case errors.Is(err, game.ErrRoundNotAcceptingGuesses), errors.Is(err, game.ErrRoundAlreadyRevealed), errors.Is(err, game.ErrRoundNotRevealed), errors.Is(err, game.ErrGuessAlreadySubmitted):
		writeError(writer, http.StatusConflict, err.Error())
