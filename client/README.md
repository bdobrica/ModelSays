...
The client contract types `gameKind` separately from pacing `mode` and understands the phase-safe trivia payload: four option IDs/labels may arrive while answering, but solution fields arrive only after reveal. The current creation form always sends `model_says` and supports individual simultaneous, team, sequential, or living-room pacing; trivia choices stay hidden until their playable UI steps are complete. It also supports 1–5 rounds, a 15–120 second timer, English locale, and the default `gpt-5.6-luna` prediction model. Room and display-name inputs are capped at the server's 48- and 24-character limits. Invite links stop accepting new players once the host starts the game.
