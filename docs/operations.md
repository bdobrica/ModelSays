...
`GET /healthz` is process-only liveness. `GET /readyz` checks PostgreSQL connectivity, clean migration version `000013`, and recent successful transition-worker processing. A failure returns only `not_ready`; details remain in private logs.
