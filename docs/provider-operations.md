...
Trivia generation has the distinct `trivia_generation` audit purpose and `trivia-v1` prompt/schema version. One call returns the question and complete frozen solution together: Open Trivia includes its canonical answer and conservative aliases, while Choice Trivia includes exactly four labeled opaque option IDs and the correct ID. Provider-authored content is bounded and validated before its integrity hash is computed or anything is persisted. Malformed, ambiguous, timed-out, over-budget, or circuit-rejected attempts retry within the shared limits and then use an unused entry from the requested kind's reviewed English bank. Five entries per kind support the maximum game length without credentials or repeats. Unsupported locales, exhausted curated pools, or failure of both paths return content unavailable rather than silently reusing or translating a question. Difficulty/category controls and external trivia APIs are deferred.
