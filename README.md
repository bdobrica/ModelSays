...
Room configuration separates game rules from pacing. The create screen offers Model Says, Open Trivia, and Choice Trivia as independent rulesets. `mode` remains individual simultaneous, team, sequential, or living-room pacing. Both trivia kinds support every exposed pacing except sequential. Team mode aggregates individual scores into 2–4 host-configured teams. Sequential mode gives every player one timed turn in lobby join order. Living-room mode turns the creator into a non-playing TV display while every human plays from a phone; Choice Trivia displays the four options on the TV while phones provide the answer controls. See [ADR-007](docs/adr/07-orthogonal_game_rulesets.md) for the compatibility matrix.
...
5. Advance, play and reveal round two, confirm all sessions show the same final ranking, open the shareable round-by-round replay, then use the host's **Play again** action and confirm the same room and players return to a clean lobby.

Without `OPENAI_API_KEY`, Model Says and both trivia kinds randomly sample without replacement from reviewed ten-question English banks and support the default 10-round flow, so ordering varies while a question never repeats within a game. Open Trivia entries freeze a canonical answer and conservative aliases; Choice Trivia entries freeze four distinct plausible options and one correct option ID. With OpenAI enabled, trivia uses its own `trivia-v1` strict schema and returns the question and complete solution atomically. All generated content is validated before persistence. Invalid or failed generation is retried once, then the server randomly selects an unused curated entry of the requested kind. Only failure of both paths returns a temporary content-unavailable response. Additional reviewed database banks can provide larger pools and other configured locales.
...
The supported room contract keeps two axes isolated: `gameKind` is `model_says`, `trivia_open`, or `trivia_choice`, while `mode` is simultaneous, teams, sequential, or living-room. Missing game kinds default to `model_says`; trivia plus sequential is currently rejected. The browser exposes typed Open Trivia and four-button Choice Trivia flows. Choice buttons preserve server order in a 2×2 grid where space permits and a single column on narrow phones; they submit only opaque option IDs and lock after the first accepted action. Trivia rounds use the frozen, versioned content contract in [ADR-008](docs/adr/08-versioned_trivia_round_content.md): choice option IDs/labels are safe during answering, while every solution field is omitted until reveal. Rooms otherwise support 1–`MAX_ROUNDS` rounds (default 10), use `DEFAULT_ROUNDS` when omitted (default 10), a 15–120 second answer timer, configured locales, and the server-configured `DEFAULT_PREDICTION_MODEL`. Room names are 3–48 Unicode characters and display names are 2–24; control characters are rejected. Room codes are six characters from the invite-code alphabet. New players may join only while the room is in the lobby; attempting to reuse an invite after start returns a conflict response. JSON mutation requests are limited to 16 KiB and reject unknown fields, trailing values, and malformed input. The browser validates its stored player token with the server after refresh and offers a clear-session action.
...
- simultaneous, team, sequential, or living-room games with a configurable round limit (10 by default) and 15–120 second timers;
...
- Team and sequential are separate modes and cannot be combined. Completed games have a non-enumerable results link, and **Play again** keeps the room, players, credentials, teams, mode, and settings while clearing scores and gameplay for newly sampled questions.
...
After completion, the room exposes a random replay identifier. Anyone with its link can view final rankings (including ties), Model Says boards/matches, trivia solutions and correctness, revealed answers or option labels, and per-round score deltas. Replay responses exclude player tokens, normalized guesses, accepted aliases, opaque option IDs in the rendered UI, provider/model/prompt metadata, audits, judge records, and raw provider content. Replays currently follow completed-game retention and do not have an automatic expiry; see [`docs/operations.md`](docs/operations.md).
...
For trivia, the TV shows the question and common timer; Choice Trivia also shows all four options in their frozen order. No solution or correctness marker appears while answering. Reveal identifies the canonical answer, submitted Choice option labels, correct players, awards, and current rankings; raw Open Trivia submissions stay on the player's own result and durable replay rather than the shared TV. The completed TV offers both **Play again**, which retains the ruleset and pacing in a clean lobby with newly sampled rounds, and **Back to home**.
