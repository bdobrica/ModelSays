...
In living-room mode, open the creator session on the shared TV and optionally select **Fullscreen**. Players scan the displayed QR (or use its ordinary join link/code) and enter names on their phones. The QR contains only the same-origin join URL—never a session token or replay/provider data. Once at least two people have joined, select **Start game**. The display cannot answer and is absent from rankings. The TV uses the full viewport for the complete question and shared timer; Choice Trivia also shows its four options in stable 2×2 order without marking the correct one. It reveals early when everyone answers or at the deadline, shows the trivia solution, Choice option labels, awards, and rankings for the configured pause, then gives the following round its full timer or completes automatically. Raw Open Trivia submissions are not repeated on the shared TV. Phones show their personal result during TV reveal and the final scoreboard at completion. On the TV scoreboard, select **Play again** to return the same connected players to a clean lobby with the same settings and newly sampled questions, or **Back to home** to leave the room.
...
When the final round is complete, use **Share results** to share or copy the unguessable replay link. The replay shows winners/ties and every revealed Model Says or trivia round, including trivia solutions, submitted answer/choice labels, correctness, and deltas, without exposing session credentials or private provider records. The host can select **Play again** to reset the existing room with the same players, teams, ruleset, pacing, and settings. No prior questions, guesses, claims, boards, deadlines, or scores carry into the new game.
